...
public class DatabaseManager {
    ...
    public DatabaseManager(Config cfg) { this.cfg = cfg; }
    ...
    public Optional<Row> load(UUID uuid) {
        try (Connection c = ds.getConnection();
             PreparedStatement ps = c.prepareStatement(
                 "SELECT nbt_base64, UNIX_TIMESTAMP(updated_at)*1000 FROM player_trinkets WHERE uuid=?"
             )) {
            ps.setString(1, uuid.toString());
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return Optional.of(new Row(rs.getString(1), rs.getLong(2)));
            }
        } catch (SQLException e) {
